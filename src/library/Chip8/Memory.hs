module Chip8.Memory
    ( Register (..)
    , Address  (Ram, Register, Pc, Sp, Dt, St)
...
toString m = do
    ...
    dt'    <- fmap prettifyWord8        . readIORef . dt $ m
    st'    <- fmap prettifyWord8        . readIORef . st $ m
...
load m St           = (readIORef . st $ m) >>= \x -> return $ Mem8 x
load m Dt           = (readIORef . dt $ m) >>= \x -> return $ Mem8 x
...
store m St (Mem8  x) = writeIORef (st m) x
store m Dt (Mem8  x) = writeIORef (dt m) x
